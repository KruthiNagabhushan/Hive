...
public class BeeHive {
    ...
    public void end() {
        ...
        /**
         * loop to end all threads at the end of simulation
         */
        for (Bee b: bees){
            if (b.isAlive()){
                try {
    ...
    public synchronized boolean hasResources() {
           return (getRemainingNectar()>=1 && getRemainingPollen()>=1);
    }
    ...
    public synchronized void deposit(Resource resource, Worker bee) {
      ...
      if (Resource.NECTAR==resource){
          nectarGathered++;
          nectar++;
      }
        if (Resource.POLLEN==resource){
           pollenGathered++;
           pollen++;
        }
    }
